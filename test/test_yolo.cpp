...
float Letterbox(const cv::Mat &image, cv::Mat &out_image,
                ...
                bool fixed_shape = false, bool scale_up = false) {
  ...
  cv::copyMakeBorder(tmp, out_image, top, bottom, left, right,
...
void ScaleCoords(const cv::Size &img_shape, cv::Rect &coords,
                 const cv::Size &img_origin_shape) {
  float gain =
      std::min((float)img_shape.height / (float)img_origin_shape.height,
               (float)img_shape.width / (float)img_origin_shape.width);

  int pad[2] = {
      (int)(((float)img_shape.width - (float)img_origin_shape.width * gain) /
            2.0f),
      (int)(((float)img_shape.height - (float)img_origin_shape.height * gain) /
            2.0f)};
...
void YoloDemo(const std::vector<std::string> &image_paths,
  ...
  for (int i = 0; i < outputs.size(); ++i) {
    ...
    cv::imwrite(std::string("../tmp/output") + std::to_string(i) + ".jpg",
                image);
  }
}
...
TEST(test_net, forward_yolo1) {
  using namespace kuiper_infer;
  RuntimeGraph graph("../tmp/yolov5n_small.pnnx.param",
                     "../tmp/yolov5n_small.pnnx.bin");
