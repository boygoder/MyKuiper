...
public:
  explicit ConvolutionOp(bool use_bias, uint32_t groups, uint32_t stride_h,
                         uint32_t stride_w, uint32_t padding_h,
                         uint32_t padding_w)
      : Operator(OpType::kOperatorConvolution), use_bias_(use_bias),
        groups_(groups), stride_h_(stride_h), stride_w_(stride_w),
