...
ExpressionLayer::ExpressionLayer(const std::shared_ptr<Operator> &op)
    : Layer("Expression") {
...
void ExpressionLayer::Forwards(
  ...
  CHECK(this->op_ != nullptr &&
        this->op_->op_type_ == OpType::kOperatorExpression);
  //用栈来模拟计算过程
  ...
  for (const auto &token_node : token_nodes) {
    ...
    if (token_node->num_index >= 0) {
      ...
      //操作数的num_index是TokenType的相反数，都小于0
      ...
      for (uint32_t i = 0; i < batch_size; ++i) {
        if (op == -int(TokenType::TokenAdd)) {
          output_token_nodes.at(i) =
              ftensor::ElementAdd(input_node1.at(i), input_node2.at(i));
        } else if (op == -int(TokenType::TokenMul)) {
          output_token_nodes.at(i) =
              ftensor::ElementMultiply(input_node1.at(i), input_node2.at(i));
        } else if (op == -int(TokenType::TokenDiv)) {
          //对应位置元素相除
          output_token_nodes.at(i) =
              ftensor::ElementDivision(input_node1.at(i), input_node2.at(i));
        } else {
