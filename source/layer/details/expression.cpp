...
InferStatus ExpressionLayer::Forward(
  ...
  if (outputs.empty()) {
    LOG(ERROR)
        << "Output and input size of the expression layer is not adapting";
  ...
  CHECK(!expressions.empty())
      << "The expression in the expression layer tokenize failed!";
...
ExpressionLayer::GetInstance(const std::shared_ptr<RuntimeOperator> &op,
