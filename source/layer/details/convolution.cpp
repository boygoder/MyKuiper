...
ConvolutionLayer::ConvolutionLayer(uint32_t output_channel, uint32_t in_channel,
    ...
    : ParamLayer("Convolution"), use_bias_(use_bias), groups_(groups),
      padding_h_(padding_h), padding_w_(padding_w), stride_h_(stride_h),
      stride_w_(stride_w) {
  if (groups != 1) {
    in_channel /= groups;
  }
...
#pragma omp parallel for schedule(dynamic)
      for (uint32_t k = 0; k < kernel_count_group; ++k) {
        ...
        memcpy(output_tensor->slice(k + g * kernel_count_group).memptr(),
...
ConvolutionLayer::GetInstance(const std::shared_ptr<RuntimeOperator> &op,
