...
#pragma omp parallel for num_threads(batch_size)
  for (uint32_t i = 0; i < batch_size; ++i) {
    ...
    CHECK(output_data.n_rows == input_data.n_rows * scale_h_)
        << "The height of the feature map is not adapting!";
    CHECK(output_data.n_cols == input_data.n_cols * scale_w_)
        << "The width of the feature map is not adapting!";
    CHECK(input_data.n_slices == output_data.n_slices)
        << "The channel of the feature map is not adapting!";
...
UpSampleLayer::GetInstance(const std::shared_ptr<RuntimeOperator> &op,
