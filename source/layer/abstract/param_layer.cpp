...
void ParamLayer::set_weights(const std::vector<float> &weights) {
  ...
  const uint32_t kernel_size = this->weights_.size();
  for (uint32_t i = 0; i < kernel_size; ++i) {
    weight_size += this->weights_.at(i)->size();
  }

  CHECK_EQ(weight_size, elem_size);
  CHECK_EQ(elem_size % kernel_size, 0);
  const uint32_t blob_size = elem_size / kernel_size;
  for (uint32_t idx = 0; idx < kernel_size; ++idx) {
    const uint32_t start_offset = idx * blob_size; // 当前卷积核参数的开始位置
    const uint32_t end_offset =
        start_offset + blob_size; // 当前卷积核参数的截至位置
...
void ParamLayer::set_bias(const std::vector<float> &bias) {
  ...
  const uint32_t kernel_size = this->bias_.size();
  for (uint32_t i = 0; i < kernel_size; ++i) {
